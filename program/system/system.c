...
void NMI_Handler(void) {
    ...
    KERNEL_DEBUG_INFO("mpec: 0x%08X    mcause: 0x%08X    mtval: 0x%08X\r\n\r\n", mepc, mcause, mtval)
    while (1) {}
}
...
void HardFault_Handler(void) {
    ...
    if ((mcause & 0x80000000UL) != 0)
        ...
        switch (mcause & 0x0000000FUL) {
            ...
            case 1:
                ...
                KERNEL_DEBUG_INFO("Address of memory access: 0x%08x\r\n", mtval)
                break;
            case 2:
                KERNEL_DEBUG_INFO("Abnormality reason: Illegal instructions\r\n");
                KERNEL_DEBUG_INFO("Illegal instruction code: 0x%08x\r\n", mtval)
                break;
            case 3:
                KERNEL_DEBUG_INFO("Abnormality reason: Breakpoints\r\n");
                KERNEL_DEBUG_INFO("PC Register value: 0x%08x\r\n", mtval)
                break;
            case 4:
    ...
    KERNEL_DEBUG_INFO("%saddr2line.exe -e %s -f 0x%x -a -p\r\n", TOOLCHAIN_PATH, PROJECT_ELF_PATH, mepc)
    while (1) {}
}

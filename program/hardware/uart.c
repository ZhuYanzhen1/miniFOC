...
uart_transmit_buffer_t transmit_buffer[UART_TRANSMIT_BUFFER_NUM] = {0};
...
unsigned char uart_transmit(const unsigned char* buffer, unsigned char length) {
    ...
    for (unsigned char counter = 0; counter < UART_TRANSMIT_BUFFER_NUM; ++counter)
        if (__builtin_expect((transmit_buffer[counter].length == 0), 1)) {
            available_buffer = &(transmit_buffer[counter]);
            break;
        }
...
void usart1_config(void) {
    ...
    for (unsigned char counter = 0; counter < UART_TRANSMIT_BUFFER_NUM; ++counter)
        transmit_buffer[counter].length = 0;
}
